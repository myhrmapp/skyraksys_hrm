import paramiko

key = paramiko.RSAKey.from_private_key_file(r"C:\Users\otyvi\.ssh\id_rsa_skyraksys")
ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect('46.225.73.94', username='Rakesh', pkey=key, timeout=30)
sftp = ssh.open_sftp()

# Upload updated .env.production
local = r"d:\skyraksys_hrm1\skyraksys_hrm_app\frontend\.env.production"
remote = "/var/www/skyraksys_hrm/frontend/.env.production"
print("Uploading updated .env.production...")
sftp.put(local, remote)
sftp.close()
print("✅ Uploaded")

# Rebuild frontend with new env vars
print("\n=== Rebuilding frontend ===")
cmd = """cd /var/www/skyraksys_hrm/frontend && \
rm -rf build && \
export NODE_OPTIONS="--max-old-space-size=4096" && \
export CI=true && \
nohup npx react-scripts build > /tmp/rebuild.log 2>&1 &
echo "Build started (PID: $!)"
"""
stdin, stdout, stderr = ssh.exec_command(cmd, timeout=30)
print(stdout.read().decode().strip())

print("\nBuild running in background. Wait ~3 minutes, then check with:")
print("  python scripts/deploy/check-rebuild.py")

ssh.close()
